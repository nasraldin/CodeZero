//  <copyright file="CodeZeroSignInManager.cs" project="CodeZero.Identity.AspNetCore" solution="CodeZero">
...
{
    public abstract class CodeZeroSignInManager<TTenant, TRole, TUser> : ITransientDependency
           where TTenant : CodeZeroTenant<TUser>
           where TRole : CodeZeroRole<TUser>, new()
           where TUser : CodeZeroUser<TUser>
    {
        public CodeZeroUserManager<TRole, TUser> UserManager { get; set; }
        ...
        private readonly ICodeZeroIdentityAspNetCoreConfiguration _configuration;
        ...
        protected CodeZeroSignInManager(
            ...
            ICodeZeroIdentityAspNetCoreConfiguration configuration)
        {
        ...
        private static ClaimsPrincipal CreateIdentityForTwoFactor(TUser user, string authType)
        {
            ...
            {
                claimsIdentity.AddClaim(new Claim(CodeZeroClaimTypes.TenantId, user.TenantId.Value.ToString()));
            }
        ...
        public virtual async Task SignInAsync(TUser user, bool isPersistent, bool? rememberBrowser = null)
        {
            using (_unitOfWorkManager.Current.SetTenantId(user.TenantId))
            {
                ...
                if (rememberBrowser == null)
                {
                    rememberBrowser = IsTrue(CodeZeroSettingNames.UserManagement.TwoFactorLogin.IsRememberBrowserEnabled, user.TenantId);
                }
        ...
        {
            var authenticateResult = await AuthenticationManager.AuthenticateAsync(_configuration.TwoFactorAuthenticationScheme);
            return CodeZeroIdentityClaimsIdentityHelper.GetTenantId(authenticateResult?.Identity);
        }
        ...
        public async Task<bool> TwoFactorBrowserRememberedAsync(string userId, int? tenantId)
        {
            ...
            if (CodeZeroIdentityClaimsIdentityHelper.GetTenantId(result.Identity) != tenantId)
            {
                return false;
            }
