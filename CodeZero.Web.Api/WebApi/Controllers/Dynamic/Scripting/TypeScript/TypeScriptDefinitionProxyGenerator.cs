//  <copyright file="TypeScriptDefinitionProxyGenerator.cs" project="CodeZero.Web.Api" solution="CodeZero">
    ...
    internal class TypeScriptDefinitionProxyGenerator : ITransientDependency
    {
        ...
        public string Generate(DynamicApiControllerInfo controllerInfo, string servicePrefix)
        {
            ...
            foreach (var methodInfo in _controllerInfo.Actions.Values)
            {
                ...
                if (returnType == "void")
                    script.AppendLine(string.Format("            {0} ({1}): CodeZero.IPromise; ", methodInfo.ActionName.ToCamelCase(), GetMethodInputParameter(methodInfo.Method)));
                else
                    script.AppendLine(string.Format("            {0} ({1}): CodeZero.IGenericPromise<{2}>; ", methodInfo.ActionName.ToCamelCase(), GetMethodInputParameter(methodInfo.Method), returnType));
