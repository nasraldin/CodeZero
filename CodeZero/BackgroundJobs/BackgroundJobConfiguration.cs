//  <copyright file="BackgroundJobConfiguration.cs" project="CodeZero" solution="CodeZero">
//      Copyright (c) 2018 CodeZero Framework.  All rights reserved.
//  </copyright>
//  <author>Nasr Aldin M.</author>
//  <email>nasr2ldin@gmail.com</email>
//  <website>https://nasraldin.com/codezero</website>
//  <github>https://nasraldin.github.io/CodeZero</github>
//  <date>01/01/2018 01:00 AM</date>
using CodeZero.Configuration.Startup;

namespace CodeZero.BackgroundJobs
{
    internal class BackgroundJobConfiguration : IBackgroundJobConfiguration
    {
        public bool IsJobExecutionEnabled { get; set; }

        public ICodeZeroStartupConfiguration CodeZeroConfiguration { get; private set; }

        public BackgroundJobConfiguration(ICodeZeroStartupConfiguration codeZeroConfiguration)
        {
            CodeZeroConfiguration = codeZeroConfiguration;

            IsJobExecutionEnabled = true;
        }
    }
}