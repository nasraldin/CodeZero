//  <copyright file="CodeZeroRoleManager.cs" project="CodeZero.Identity" solution="CodeZero">
...
namespace CodeZero.Authorization.Roles
{
    ...
    /// </summary>
    public abstract class CodeZeroRoleManager<TRole, TUser>
        ...
        where TUser : CodeZeroUser<TUser>
    {
        ...
        private IRolePermissionStore<TRole> RolePermissionStore
        {
            get
            {
                ...
                {
                    throw new CodeZeroException("Store is not IRolePermissionStore");
                }
        ...
        protected CodeZeroRoleStore<TRole, TUser> CodeZeroStore { get; private set; }
        ...
        public async override Task<IdentityResult> DeleteAsync(TRole role)
        {
            if (role.IsStatic)
            {
                return CodeZeroIdentityResult.Failed(string.Format(L("CanNotDeleteStaticRole"), role.Name));
            }
        ...
        /// <exception cref="CodeZeroException">Throws exception if no role with given id</exception>
        public virtual async Task<TRole> GetRoleByIdAsync(int roleId)
        {
            var role = await FindByIdAsync(roleId);
            if (role == null)
            {
                throw new CodeZeroException("There is no role with id: " + roleId);
            }
        ...
        /// <param name="roleName">Role name</param>
        /// <returns>Role</returns>
        /// <exception cref="CodeZeroException">Throws exception if no role with given roleName</exception>
        public virtual async Task<TRole> GetRoleByNameAsync(string roleName)
        {
            var role = await FindByNameAsync(roleName);
            if (role == null)
            {
                throw new CodeZeroException("There is no role with name: " + roleName);
            }
        ...
        public virtual async Task<IdentityResult> CheckDuplicateRoleNameAsync(int? expectedRoleId, string name, string displayName)
        {
            ...
            {
                return CodeZeroIdentityResult.Failed(string.Format(L("RoleNameIsAlreadyTaken"), name));
            }

            role = await FindByDisplayNameAsync(displayName);
            if (role != null && role.Id != expectedRoleId)
            {
                return CodeZeroIdentityResult.Failed(string.Format(L("RoleDisplayNameIsAlreadyTaken"), displayName));
            }
