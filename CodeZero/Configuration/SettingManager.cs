//  <copyright file="SettingManager.cs" project="CodeZero" solution="CodeZero">
...
namespace CodeZero.Configuration
{
    ...
    public class SettingManager : ISettingManager, ISingletonDependency
    {
        ...
        {
            return GetSettingValueInternalAsync(name, CodeZeroSession.TenantId, CodeZeroSession.UserId);
        }
        ...
        public async Task<IReadOnlyList<ISettingValue>> GetAllSettingValuesAsync(SettingScopes scopes)
        {
            ...
            if (scopes.HasFlag(SettingScopes.Application))
            {
                foreach (var settingValue in await GetAllSettingValuesForApplicationAsync())
                {
                    ...
                    if (!setting.IsInherited &&
                        ((setting.Scopes.HasFlag(SettingScopes.Tenant) && CodeZeroSession.TenantId.HasValue) || (setting.Scopes.HasFlag(SettingScopes.User) && CodeZeroSession.UserId.HasValue)))
                    {
                        continue;
                    }
            ...
            if (scopes.HasFlag(SettingScopes.Tenant) && CodeZeroSession.TenantId.HasValue)
            {
                foreach (var settingValue in await GetAllSettingValuesForTenantAsync(CodeZeroSession.TenantId.Value))
                {
                    var setting = settingDefinitions.GetOrDefault(settingValue.Name);
                    ...
                    if (!setting.IsInherited &&
                        (setting.Scopes.HasFlag(SettingScopes.User) && CodeZeroSession.UserId.HasValue))
                    {
                        continue;
                    }
            ...
            if (scopes.HasFlag(SettingScopes.User) && CodeZeroSession.UserId.HasValue)
            {
                foreach (var settingValue in await GetAllSettingValuesForUserAsync(CodeZeroSession.ToUserIdentifier()))
                {
                    var setting = settingDefinitions.GetOrDefault(settingValue.Name);
        ...
        {
            return GetAllSettingValuesForUserAsync(new UserIdentifier(CodeZeroSession.TenantId, userId));
        }
        ...
        {
            return ChangeSettingForUserAsync(new UserIdentifier(CodeZeroSession.TenantId, userId), name, value);
        }
