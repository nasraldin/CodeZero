//  <copyright file="CodeZeroAppServiceConvention.cs" project="CodeZero.AspNetCore" solution="CodeZero">
...
namespace CodeZero.AspNetCore.Mvc.Conventions
{
    public class CodeZeroAppServiceConvention : IApplicationModelConvention
    {
        private readonly Lazy<CodeZeroAspNetCoreConfiguration> _configuration;

        public CodeZeroAppServiceConvention(IServiceCollection services)
        {
            _configuration = new Lazy<CodeZeroAspNetCoreConfiguration>(() =>
            {
                return services
                    .GetSingletonService<CodeZeroBootstrapper>()
                    .IocManager
                    .Resolve<CodeZeroAspNetCoreConfiguration>();
            }, true);
        }
        ...
        private void ConfigureArea(ControllerModel controller, [CanBeNull] CodeZeroControllerAssemblySetting configuration)
        {
        ...
        private void ConfigureRemoteService(ControllerModel controller, [CanBeNull] CodeZeroControllerAssemblySetting configuration)
        {
        ...
        private void ConfigureSelector(ControllerModel controller, [CanBeNull] CodeZeroControllerAssemblySetting configuration)
        {
        ...
        private void ConfigureSelector(string moduleName, string controllerName, ActionModel action, [CanBeNull] CodeZeroControllerAssemblySetting configuration)
        {
            ...
            {
                AddCodeZeroServiceSelector(moduleName, controllerName, action, configuration);
            }
            else
            {
        ...
        private void AddCodeZeroServiceSelector(string moduleName, string controllerName, ActionModel action, [CanBeNull] CodeZeroControllerAssemblySetting configuration)
        {
            var CodeZeroServiceSelectorModel = new SelectorModel
            {
                AttributeRouteModel = CreateCodeZeroServiceAttributeRouteModel(moduleName, controllerName, action)
            };
            ...
            CodeZeroServiceSelectorModel.ActionConstraints.Add(new HttpMethodActionConstraint(new[] { verb }));

            action.Selectors.Add(CodeZeroServiceSelectorModel);
        }
        ...
        private static void NormalizeSelectorRoutes(string moduleName, string controllerName, ActionModel action)
        {
            foreach (var selector in action.Selectors)
            {
                ...
                {
                    selector.AttributeRouteModel = CreateCodeZeroServiceAttributeRouteModel(
                        moduleName,
                        controllerName,
                        action
                    );
        ...
        private string GetModuleNameOrDefault(Type controllerType)
        {
            return GetControllerSettingOrNull(controllerType)?.ModuleName ??
                   CodeZeroControllerAssemblySetting.DefaultServiceModuleName;
        }

        [CanBeNull]
        private CodeZeroControllerAssemblySetting GetControllerSettingOrNull(Type controllerType)
        {
            return _configuration.Value.ControllerAssemblySettings.GetSettingOrNull(controllerType);
        }

        private static AttributeRouteModel CreateCodeZeroServiceAttributeRouteModel(string moduleName, string controllerName, ActionModel action)
        {
