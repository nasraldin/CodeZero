//  <copyright file="LdapAuthenticationSource.cs" project="CodeZero.Identity.Ldap" solution="CodeZero">
...
namespace CodeZero.Identity.Ldap.Authentication
{
    /// <summary>
    ...
    public abstract class LdapAuthenticationSource<TTenant, TUser> : DefaultExternalAuthenticationSource<TTenant, TUser>, ITransientDependency
        ...
        where TUser : CodeZeroUserBase, new()
    {
        /// <summary>
        /// LDAP
        /// </summary>
        ...
        private readonly ICodeZeroLdapModuleConfig _ldapModuleConfig;
        ...
        public override async Task<TUser> CreateUserAsync(string userNameOrEmailAddress, TTenant tenant)
        {
            ...
            using (var principalContext = await CreatePrincipalContext(tenant, user))
            {
                ...
                {
                    throw new CodeZeroException("Unknown LDAP user: " + userNameOrEmailAddress);
                }
        ...
        public override async Task UpdateUserAsync(TUser user, TTenant tenant)
        {
            ...
            using (var principalContext = await CreatePrincipalContext(tenant, user))
            {
                ...
                if (userPrincipal == null)
                {
                    throw new CodeZeroException("Unknown LDAP user: " + user.UserName);
                }
        ...
        protected virtual async Task CheckIsEnabled(TTenant tenant)
        {
            ...
            {
                throw new CodeZeroException("Ldap Authentication module is disabled globally!");                
            }

            var tenantId = tenant?.Id;
            if (!await _settings.GetIsEnabled(tenantId))
            {
                throw new CodeZeroException("Ldap Authentication is disabled for given tenant (id:" + tenantId + ")! You can enable it by setting '" + LdapSettingNames.IsEnabled + "' to true");
            }
        }
